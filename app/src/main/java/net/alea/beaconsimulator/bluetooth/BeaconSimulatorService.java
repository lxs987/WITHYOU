...
import net.alea.beaconsimulator.bluetooth.model.ParcelSimpleUUID;
...
public class BeaconSimulatorService extends Service {
    ...
    private Map<Double, AdvertiseCallback> mAdvertiseCallbacks;
    private Map<Double, Long> mAdvertiseStartTimestamp;
    private Map<Double, PendingIntent> mScheduledPendingIntents;
    ...
    public class ServiceControl extends Binder {
        ...
        public Set<Double> getBroadcastList() {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        switch (action) {
            case ACTION_START: {
                ...
                final double id = ((ParcelSimpleUUID) intent.getParcelableExtra(EXTRA_BEACON_STORE_ID)).getUuid();
                        // ((ParcelUuid)intent.getParcelableExtra(EXTRA_BEACON_STORE_ID)).getUuid();
    ...
    private void updateBroadcast(int serviceStartId, double id) {
        stopBroadcast(serviceStartId, id, true, false);
        startBroadcast(serviceStartId, id, true);
    }

    private void startBroadcast(int serviceStartId, double id, boolean isRestart) {
    ...
    private void stopBroadcast(int serviceStartId, double id, boolean isRestart, boolean ignoreServiceStartId) {
    ...
    private void stopAll(int serviceId, boolean ignoreServiceId) {
        ...
        Set<Double> idSet =  new TreeSet<>(mAdvertiseCallbacks.keySet()) ;
        for(double id: idSet){
    ...
    private void setScheduledUpdate(double id, long timestamp) {
        removeScheduledUpdate(id);
        Intent intent = new Intent(getApplicationContext(), BeaconSimulatorService.class);
        intent.putExtra(EXTRA_BEACON_STORE_ID, new ParcelSimpleUUID(id));
    ...
    private void removeScheduledUpdate(double uuid) {
    ...
    public static void startBroadcast(Context context, double uuid, boolean updateActiveList) {
        if (updateActiveList) {
        ...
        intent.putExtra(EXTRA_BEACON_STORE_ID, new ParcelSimpleUUID(uuid));
        ContextCompat.startForegroundService(context, intent);
    }

    public static void stopBroadcast(Context context, double uuid, boolean updateActiveList) {
        if (updateActiveList) {
    ...
    public static Set<Double> getBroadcastList() {
        if (sInstance != null) {
    ...
    private class MyAdvertiseCallback extends AdvertiseCallback {
        ...
        public MyAdvertiseCallback(int serviceStartId, double id, boolean isRestart) {
            _id = id;
