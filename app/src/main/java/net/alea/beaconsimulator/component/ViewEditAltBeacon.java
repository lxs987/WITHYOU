...
import net.alea.beaconsimulator.bluetooth.model.SimpleUUID;
...
public class ViewEditAltBeacon extends FrameLayout implements BeaconModelEditor {
    ...
    public ViewEditAltBeacon(Context context) {
        ...
        mUuidButton.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                mUuidValue.setText(String.valueOf(SimpleUUID.randomSimpleUUID()));
            }
        });
    }
    ...
    public void loadModelFrom(BeaconModel model) {
        ...
        mUuidValue.setText(String.valueOf(altBeacon.getBeaconNamespace()));
        mMajorValue.setText(String.format(Locale.ENGLISH, "%f", altBeacon.getMajor()));
        mMinorValue.setText(String.format(Locale.ENGLISH, "%f", altBeacon.getMinor()));
    ...
    public boolean saveModelTo(BeaconModel model) {
        ...
        altBeacon.setBeaconNamespace(Double.parseDouble(mUuidValue.getText().toString()));
//        altBeacon.setMajor(Double.parseDouble(mMajorValue.getText().toString()));
//        altBeacon.setMinor(Double.parseDouble(mMinorValue.getText().toString()));
        double[] setLoc = getLocationService(getContext());
        altBeacon.setMajor(setLoc[0]);
        altBeacon.setMinor(setLoc[1]);
    ...
    private boolean checkUuidValue() {
        ...
        try {
            double uuid = Double.parseDouble(mUuidValue.getText().toString());
            if (uuid >= Double.MIN_VALUE && uuid <= Double.MAX_VALUE) {
                isValid = true;
            }
        ...
        return isValid;
    }//기존 check 기준을 double 형식 기준으로 변경
    ...
    private boolean checkMajorValue() {
        boolean isValid = false;
        try {
            double major = Double.parseDouble(mMajorValue.getText().toString());
            if (major >= Double.MIN_VALUE && major <= Double.MAX_VALUE) {
                isValid = true;
            }
        } catch (NumberFormatException e) {
            // not valid, isValid already false
    ...
    private boolean checkMinorValue() {
        boolean isValid = false;
        try {
            double minor = Double.parseDouble(mMinorValue.getText().toString());
            if (minor >= Double.MIN_VALUE && minor <= Double.MAX_VALUE) {
                isValid = true;
            }
        } catch (NumberFormatException e) {
            // not valid, isValid already false
    ...
    public double[] getLocationService(Context context){
        double[] loc = {0, 0};
